...
async function runConversation(V, seed, corpus, methodCore, replayTurns = null) {
  ...
  return {
    seed,
    // 🔴 THE DIALOGUE ITSELF, carried out so it can be READ (7 September 2026). This function computed a
    // dozen aggregates and threw the conversation away, which was correct while the tool was the thing
    // being measured. Since the dialogue became the artifact, discarding it discards the artifact and
    // keeps only statistics about it — so a replay could be scored and never read.
    history,
...
// Emits the replay as a zetizeti dialogue in the SAME shape buildTranscriptMd writes, so the engagement
// reader takes it with no special case: front matter, `**Q.**` for the stone, everything else the
// learner. One format, two producers — a second shape would drift.
function emitTranscript(history, goal, outPath) {
  let body = '';
  for (const h of history) {
    body += (h.role === 'stone' ? '**Q.** ' + String(h.content).replace(/\n+/g, ' ') : String(h.content)) + '\n\n';
  }
  const topic = String(goal).replace(/\s+/g, ' ').slice(0, 110);
  const md = '---\nsource: zetizeti\ntype: idea-transcript\ntopic: ' + topic
    + '\ndate: ' + new Date().toISOString()
    + '\nturns: ' + history.filter((h) => h.role !== 'stone').length + '\n---\n\n'
    + '# A dialogue — ' + topic + '\n\n'
    + '_A replay: a real learner\'s replies, with questions generated by this build. Not a conversation that\n'
    + 'happened — the replies are theirs and the questions are new._\n\n' + body;
