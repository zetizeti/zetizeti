...
// then runs `--accept`. The live parity test (test/sensed.test.mjs, layer 3) is the proof that
...
function runParity() {
  const res = spawnSync('node', ['--test', 'test/sensed.test.mjs'], { cwd: APP, encoding: 'utf8' });
...
async function accept() {
  ...
  const manifest = {
    ...
    note: 'lib/sensed.mjs verified against this rules.py by the live cross-check in test/sensed.test.mjs. The MCP is canonical; the port references it.',
  };
