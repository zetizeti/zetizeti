...
Prayas: *"figure fixes for"* the four faults the 9 September reading left open: the rut that moved from the construction to the noun, no move for *I cannot parse that*, 62 of 458 questions shipping flagged after four generations, and praise before asking.

### Asking back — a third footing beside decline and correction

A learner who could not follow the question said so — *what*, *what do you mean by better?*, *pls explain*, *i cant understand you pls explain*, *idk what is diff in ending and goodbye* — and every time was given a further question resting on the thing they could not follow. `isDecline` reads *i don't know* and changes footing to earlier material, which is right for somebody with nothing to say and wrong for somebody who wants the SAME thing asked plainer. `isAskingBack` in `lib/arc.mjs` is structural, not a phrase list: the reply is interrogative — a question mark, an interrogative in its first three words, or a request to explain — AND adds not one content word of its own beyond the tool's last three questions and the few words a person uses to ask what was meant. It outranks the decline (*idk what is diff…* declines and asks, and the ask is the half that can be acted on). The steering block asks the same thing again under fifteen words, out of words they have used, defines nothing and explains nothing — invariant #3 untouched. Measured over 438 real replies before it was written: nine fire, none an answer; *positive or negative?* and *without changing the habit?* add a word and deliberately do not.

### The rut — hand the subject back, once per six

v0.28.0's head ban works and the metronome moved down a level: twenty questions recombining four of one learner's nouns with a different interrogative each time, runs of six to fifteen on one word across ten of the twenty dialogues. `readRut` reads one content word, exactly, across consecutive questions — the reading the probe has printed as *longest rut* since 17 August — and fires the same move `stalled` fires: invite them to name another part, their word, no candidates. 🔴 **It is not a frequency ban** (the 17 August argument stands: banning the project's central noun is worse than the rut) **and it is not the semantic re-ask gate refused on 6 September** (that looked for a repeated request by similarity, where everything in a focused conversation is similar by construction). `RUT_TURNS = 6` — the cohort median, one above the 29 July session's longest run, so it does not fire on the student the other readings were tuned on. 🔴 **And it fires at six, twelve, eighteen — never on every turn past six.** The first cut re-fired each turn, and on a replay of a learner whose project *is* the noun the invitation kept the noun, the run never broke, and seven invitations went out in a row. A guard whose satisfying set has a cheapest member is met by that member, arriving in the tool's own hand-back.

### The guard's last attempt may change footing

62 of 458 delivered questions had breached after all four generations, 33 for repeating a frame. The retries were working as written and could not succeed: every correction said *keep the subject; change the form*, and in a rut the subject is what has run out of forms. `generateGuarded` now takes an optional `fallback` — a second generate/validate pair that hands the subject back (the rut invite's exhausted form, no anchor, no join, no aim) — used on the last attempt only, only after every ordinary attempt breached, ranked against the others so a fallback that also breaches does not ship merely for being last. The retry budget is unchanged; what changes is what its last unit buys. The client is told `fallback: true` and which `footing` the turn took.

### No verdicts in the warmth clause — the 24 July dial is retired

Eight preambles in twenty dialogues valued the learner's material — *is doing real work* ×3, *is opening something* ×3, *is becoming clearer*, *is opening the way you see* — and every one was the system prompt's or the warmth posture's own example sentence said back. Both instructions are gone; what survives is the acknowledgement, their own words said back and then the question, which is what `FLOW_SHAPES[3]` had said all along. Seven appraisal constructions are refused at the guard on both surfaces, never acquitted by a word the learner used — them having said *work* does not license *is doing real work*. This retires the *light encouragement* setting Prayas asked for on 24 July after two testers found the tool too strict; under the artifact framing a verdict on what somebody's material is doing is a judgement on the material, and the judging is theirs.

### The probe had been feeding the route its own reply twice

🔴 `/api/chat` appends `message` to the student turns it reads from `history`, and the browser sends `history.slice(0, -1)` for that reason. `enquiry-conversation-probe.mjs` sent the whole history, so from 17 August every reply reached the route twice, `newMaterial` was empty on every turn after the first, `stalled` was true throughout — precision off in every probe run, and from 6 September the stalled invite firing on every turn. Found only because a footing count was added and read `stalled-invite×33` of 37. **The 6 September measurement of the head ban — twelve distinct heads, zero breaches — was taken on a tool handing the subject back on every turn**, which is a shape that produces exactly that result. Production was never affected. Every conversation-probe number before 9 September is a number about a route being fed its own reply twice.

### Measured — real replies, real route, and what a replay cannot say

`--replay=<fixture>` feeds a real learner's replies to the real route, which generates new questions with today's build. Under Node 20, in an out-of-tree copy. Baseline (v1.3.1): fixture d — rut 10 on *goodbye*, 11 breaches · fixture s — rut 10 on *trust*, 0 · fixture q — rut 8 on *user*, 9 · the 29 July student — rut 3, 5. After: fixture d — **rut 4, 7 breaches**, asking-back ×3, rut invite ×1, two hand-backs · fixture s — **rut 7**, rut invite ×1, 3 breaches, three hand-backs (the first cut, re-firing every turn, read rut 14 and seven invitations). ⚠️ **The after-runs for fixture q and the 29 July student were lost to the harness twice and not re-run**, so the after side is two fixtures and the baseline four. ⚠️ **A fixed reply cannot answer an invitation** — a replayed learner keeps saying what they said to a different question — so a replay overstates ruts on the after side and says nothing about whether the questions are better. Read the footing counts and the breach counts from it; never its questions as a dialogue. 437 tests pass under Node 20, none under Node 24, where `better-sqlite3` aborts the process at exit and, in a running server, mid-conversation.
