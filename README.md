...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/keys-BYOK%2C%20never%20stored-0f7d72?style=flat-square" alt="BYOK, keys never stored">
  <img src="https://img.shields.io/badge/model-Claude%20only-173a8c?style=flat-square" alt="Claude only">
</p>
...
The division of labour is fixed and literal (Split-Domain Cognition):
**the AI does language · code does judgement and tracking · the human decides.**

```text
Socratic voice
  learner names an edge → exact-word FTS5 retrieval (verified corpus, no embeddings)
  → Claude composes ONE question → never-answer guard (deterministic code) → the learner thinks

Critique voice
  student pastes an idea → qualify: segment + SDC-tag (deterministic, NO LLM, every tag carries a why)
  → sensed reading locates the blur (deterministic split-ratio arithmetic, parity-tested)
  → Claude phrases ONE question at the located spot → verdict-drift guard (code) → the student judges
```
...
The full data-flow contract is in [`GOAL-AND-CURTAIN-LOGIC.md`](GOAL-AND-CURTAIN-LOGIC.md); the
design rationale in [`docs/concept/architecture.md`](docs/concept/architecture.md); the core commitment in
...
The complete working records — build trackers, citation ledgers, adversarial-pass findings, sign-off
sheets — are released in [`docs/corpus-build/`](docs/corpus-build/), because an auditable corpus should be
...
- **Keys are never stored or logged** — your API key exists in-flight for one request, then is gone.

## Running it

**Use it now:** sign in with Google at [zetizeti.com](https://zetizeti.com). A small free daily allowance
is available; beyond it you bring your own [OpenRouter](https://openrouter.ai) key (stored only in your
browser, sent per-request, never persisted server-side). The model is always Claude.
...
node --test                   # the suite, incl. the guards and the locator parity tests
```

**Self-hosting:** the app is a single small Node 20 service (Express + SQLite, FTS5 built in) with a
clean [`Dockerfile`](Dockerfile). A school can run it pure-BYOK — in that configuration the database
holds **no keys at all**. Secrets enter only as environment variables; see
the [`Dockerfile`](Dockerfile) and `captain-definition` for the container shape.
...
| [`GOAL-AND-CURTAIN-LOGIC.md`](GOAL-AND-CURTAIN-LOGIC.md) | the front-end ⇄ back-end contract |
| [`docs/corpus-build/`](docs/corpus-build/) | the full verification protocols, trackers, citation ledgers, sign-off sheets |
