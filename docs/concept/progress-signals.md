...
betraying `position.md`. Read with `position.md` (the core commitment), `GOAL-AND-CURTAIN-LOGIC.md`
§2 (progress is code-owned, never an AI verdict), and `app/build-plan.md` (the anti-gamification
...
- **Not a score, percentage, badge, streak, or comparison.** That apparatus is what converts a
  ...
  refuse (`build-plan.md`, Tension 2).
...
`GOAL-AND-CURTAIN-LOGIC.md` §2 (no score drives any display):
...
- **`GOAL-AND-CURTAIN-LOGIC.md` §2:** any backend progress signal is *advisory only and must not
