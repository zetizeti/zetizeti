...
### 5.3b The speccing surface — a third face, and its polarity is neither of the others' (v0.25.0, 2 September 2026)

**Enquiry questions an idea the student holds. Criticism unpacks a text she brought and did not write. The speccing surface questions a SPECIFICATION** — language written so that something can be built from it — and the student's own document is the OBJECT, unfinished on purpose and expected to change while the conversation runs.
...
⚠️ **`focus:'concept'` does not compose with this surface** and no toggle is offered. The making filter drops the corpus entries marked *making*, which is the entire register a specification lives in — the same non-composition already settled for document mode, arriving for a different reason.
