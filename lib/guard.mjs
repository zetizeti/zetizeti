...
export function repairInstruction(reasons = [], { mode = 'enquiry', attempt = 1, lead = false } = {}) {
  ...
  const escalation = attempt >= 2
    ...
    : '';
  // A RETURN needs one sentence before the question (v1.11.0). Without this the repair itself — "ONE short
  // question only — a single sentence", "ONE clause" — orders the lead-in away on every retry.
  if (lead) {
    const shape = attempt >= 2 ? ' This is the second correction: keep the question itself to one clause and open it with a different word.' : '';
    return `That reply broke this mode's rule (${why}).${shape} Reply again with ONE short sentence, under 22 words, ending in a full stop, saying only what their last answer has not yet said — the gap you were given, in a word from your last question, without restating their answer — then ONE short open question asking only for that, beginning with What, Where, When, Which, Who or How. Do not grade their answer, and do not explain, advise or answer.`;
  }
...
// 🔴 THE FINAL ATTEMPT MAY CHANGE FOOTING (9 September 2026). Twenty real dialogues, 458 delivered
// questions, 62 of them flagged after all four generations — 33 for repeating the frame of an earlier
// question. The retries were working as written and could not succeed: every correction said "keep the
// subject; change the form", and in a rut the subject is what has run out of forms. Three corrections on
// an exhausted subject produce three differently-broken questions and the least-bad one ships flagged.
// So the caller may supply `fallback` — a second { generate, validate } pair that hands the SUBJECT back
// (the rut invite in dialogue.mjs) — and it is used on the last attempt only, and only after every
// ordinary attempt has breached. The retry budget is unchanged; what changes is what the last unit of it
// is spent on. The fallback's candidate competes on the same ranking as the others, so a fallback that
// also breaches does not ship merely for being last. `fallback: true` on the result says which shipped.
export async function generateGuarded({ generate, validate, attempts = 4, mode = 'enquiry', fallback = null, lead = false }) {
  let best = null;
  for (let attempt = 0; attempt < attempts; attempt++) {
    const useFallback = !!(fallback && attempt === attempts - 1 && best);
    const gen = useFallback ? fallback.generate : generate;
    const val = useFallback ? fallback.validate : validate;
    const correction = attempt === 0 ? null : {
      previous: best?.text || '',
      reasons: best?.check?.reasons || [],
      // the fallback hands the subject back, so it never carries the return's lead-in
      instruction: repairInstruction(best?.check?.reasons || [], { mode, attempt, lead: lead && !useFallback }),
    };
    ...
    const text = plainQuestion((await gen(correction)) ?? '');
    const check = val(text);
    const candidate = { text, check, fallback: useFallback };
