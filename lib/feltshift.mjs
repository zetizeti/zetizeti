...
export function readFeltShifts({
  ...
  for (const item of sequence) {
    ...
    turns.push({
      ...
      // ADDITIVE (v0.10.2): how many items the turn offered. `semGain` is a MASS — it grows with the
      // length of the reply — so the semantic freshness channel (lib/novelty.mjs) needs the denominator
      // to read it as a RATE. Nothing else consumes this; the existing fields are untouched.
      items: scored.length,
