...
db.exec(`
  ...
  -- (see also turn_depth below — the survival curve, which is counts with no user column at all.)
  ...
  -- TURN DEPTH — the survival curve (30 Jul 2026). The one thing this project could never see was a
  -- learner STOPPING. Both probe axes read a completed transcript, and pool_spend is keyed (day, user),
  -- so one ten-turn conversation and five two-turn conversations were the same row. The two clearest
  -- signals zetizeti ever received — a student stopping at a manufactured association bridge, another at
  -- a two-box menu — arrived as messages from people who happened to mention it.
  --
  -- This needs NO session identifier and NO user column, because the service is stateless: the browser
  -- posts the whole transcript each turn, so every request already carries how deep into a conversation
  -- it is. One row per (day, surface, version, depth) counting how many turns ever reached that depth.
  -- The DROP between depth N and N+1 is exactly how many conversations ended at N.
  --
  -- It is strictly MORE private than the ledger beside it: no user_id, no session id, no content, no way
  -- to associate two rows with one person. A count, which is the category the ephemeral promise already
  -- keeps ("your words are never written"; the turn counts always were). Therefore OPERATIONAL, and NOT
  -- purged on boot — purging it would destroy the only cross-release comparison the project has.
  CREATE TABLE IF NOT EXISTS turn_depth (
    day     TEXT NOT NULL,               -- 'YYYY-MM-DD' (UTC)
    surface TEXT NOT NULL,               -- 'enquiry' | 'criticism'
    version TEXT NOT NULL,               -- the build that asked, so releases can be compared
    depth   INTEGER NOT NULL,            -- 1-based: this turn was the Nth of its conversation
    count   INTEGER NOT NULL DEFAULT 0,
    PRIMARY KEY (day, surface, version, depth)
  );
...
// ── SUBMITTED DIALOGUES (7 September 2026) ────────────────────────────────────────────────────────
// 🔴 THIS IS AN UPLOAD, NOT TELEMETRY, AND THE DISTINCTION IS THE WHOLE SAFETY OF IT. Nothing is
// captured; something is SENT. A resident marks a dialogue they already hold — the service never had a
// copy — and deposits it deliberately. `lib/capture.mjs` remains the only path that could retain a
// conversation on its own and it still cannot arm itself in production.
//
// 🔴 OUTSIDE THE BOOT PURGE, on turn_depth's precedent and for a stronger reason: a purged submission is
// a person's work destroyed. It is deposited on purpose and it is theirs; the ephemeral pivot is about
// what the service keeps OF ITS OWN ACCORD, and this is not that.
//
// 🔴 WITHDRAWAL IS NOT A NEGOTIATION. `withdrawSubmission` deletes; there is no soft-delete, no flag, no
// archive. A tombstone that keeps the text is not a withdrawal.
db.exec(`
  CREATE TABLE IF NOT EXISTS submissions (
    id          TEXT PRIMARY KEY,
    user_id     TEXT NOT NULL,
    email       TEXT NOT NULL,      -- who deposited it, so they can withdraw it without proving anything
    filename    TEXT,
    body        TEXT NOT NULL,      -- the marked dialogue, exactly as they sent it
    bytes       INTEGER NOT NULL,
    created_at  INTEGER NOT NULL
  );
  CREATE INDEX IF NOT EXISTS submissions_user ON submissions (user_id, created_at DESC);
`);

const _insSubmission = db.prepare(
  `INSERT INTO submissions (id, user_id, email, filename, body, bytes, created_at)
   VALUES (@id, @user_id, @email, @filename, @body, @bytes, @created_at)`);
const _submissionsForUser = db.prepare(
  'SELECT id, filename, bytes, created_at FROM submissions WHERE user_id = ? ORDER BY created_at DESC');
const _submissionOwned = db.prepare('SELECT id, user_id FROM submissions WHERE id = ?');
const _delSubmission = db.prepare('DELETE FROM submissions WHERE id = ? AND user_id = ?');

export function addSubmission(userId, email, { filename, body }) {
  const row = { id: id('s_'), user_id: userId, email: String(email || '').toLowerCase(),
    filename: filename ? String(filename).slice(0, 200) : null,
    body: String(body), bytes: Buffer.byteLength(String(body), 'utf8'), created_at: now() };
  _insSubmission.run(row);
  return { id: row.id, bytes: row.bytes, created_at: row.created_at };
}
// 🔴 NEVER returns `body`. A listing is for the depositor to see what they have sent and withdraw it;
// reading a dialogue is a separate act done from the machine, not through an endpoint.
export function listSubmissions(userId) { return _submissionsForUser.all(userId); }
export function withdrawSubmission(userId, subId) {
  const row = _submissionOwned.get(subId);
  if (!row || row.user_id !== userId) return false;
  return _delSubmission.run(subId, userId).changes > 0;
}
...
// turn_depth — identity, the metering that backs the caps and admin monitor, and the survival curve;
// never conversation) are preserved, AND `submissions` — deposited on purpose by the person who owns
// them, so purging it would destroy somebody's work rather than enforce anything. turn_depth in
// particular MUST NOT be added to the purge either: it holds no
// user, no session and no text, and purging it would destroy the only cross-release retention comparison
// the project has.
try {
...
// The maintenance identity (26 August 2026, Prayas: *"keep a mantainence door open for you in all apps"*
// and *"remove gates for mantainence"*). A singleton account minted ONLY by presenting the bearer token
// in ZETIZETI_MAINT_TOKEN, so the operator can reach the admin surfaces without a Google round-trip and
// without being on ZETIZETI_ADMIN_EMAILS. Unlike the guest account this one DOES exist in production —
// that is the whole point of it — which is why the token is the only thing standing in front of it.
//
// 🔴 It is recognised by its google_sub, not by a new column, so this needs no migration and cannot be
// confused with a real Google account: '__maint__' is not a value Google will ever issue.
// 🔴 It spends nothing. The maintenance email is on no cohort allowlist, so tierForUser puts it in NONE
// and every generation path refuses it. Admin surfaces are readable; the wallets are not reachable.
const MAINT_SUB = '__maint__';
export const MAINT_EMAIL = 'maintenance@zetizeti.local';
export function getOrCreateMaint() {
  const existing = _userBySub.get(MAINT_SUB);
  if (existing) return existing;
  const row = { id: id('u_'), google_sub: MAINT_SUB, email: MAINT_EMAIL, name: 'Maintenance', picture: null, is_guest: 0, created_at: now() };
  _insUser.run(row);
  return row;
}
export function userIsMaint(u) { return !!u && u.google_sub === MAINT_SUB; }
...
// ttlMs is optional and defaults to the 30-day session every human sign-in gets. A maintenance session
// passes a short one deliberately: a door that stands open for a month is a door, not a visit.
export function createSession(userId, ttlMs = SESSION_TTL) {
  const token = randomBytes(24).toString('hex');
  _insSession.run(token, userId, now(), now() + ttlMs);
  return token;
}
...
// `discipline` is no longer threaded (removed 7 Sep 2026). The COLUMN stays with its DEFAULT 'all' —
// dropping it would be a migration on live data for no gain.
export function createCriticismChat(userId, { artefact, goal = '', reading = null }) {
  const row = {
    id: id('c_'), user_id: userId, title: titleFromArtefact(artefact), artefact,
    discipline: 'all', goal: goal || '',
...
// ---- turn depth: the survival curve (30 Jul 2026) ----
// Recorded on a DELIVERED turn only. A refused turn (no access, cap hit, empty generation) is not a turn
// the learner had, and counting it would inflate exactly the depth where people actually leave.
const _depthUpsert = db.prepare(
  `INSERT INTO turn_depth (day, surface, version, depth, count) VALUES (@day, @surface, @version, @depth, 1)
   ON CONFLICT(day, surface, version, depth) DO UPDATE SET count = count + 1`
);
// `depth = 0` and NOT `= 1` is deliberate. A destructuring default of 1 would turn a caller passing
// `depth: undefined` — a field that did not exist, a rename, a refactor — into a silently recorded depth-1
// turn, inflating the first bucket and making retention look worse than it is. A missing depth is a
// programming error; the right behaviour is to record nothing and let the bucket stay empty. (Caught by
// the unit test, not by reading: every other nonsense value already failed the range check.)
export function noteTurnDepth({ day, surface = 'enquiry', version = 'unknown', depth = 0 } = {}) {
  const d = Math.floor(Number(depth) || 0);
  if (d < 1 || d > 500) return;                 // 500 is a runaway guard, not a real ceiling
  _depthUpsert.run({ day, surface, version, depth: d, count: 1 });
}

// The curve itself: how many turns reached each depth, and — the number this exists for — what share of
// conversations that got to depth N did not come back for N+1. `reached` is monotonically non-increasing
// in a healthy dataset; `leftHere` is the drop to the next depth, which is where a question lost someone.
const _depthRows = db.prepare(
  `SELECT depth, SUM(count) AS reached FROM turn_depth
   WHERE surface = @surface AND (@version IS NULL OR version = @version)
     AND (@sinceDay IS NULL OR day >= @sinceDay)
   GROUP BY depth ORDER BY depth ASC`
);
export function turnDepthCurve({ surface = 'enquiry', version = null, sinceDay = null } = {}) {
  const rows = _depthRows.all({ surface, version, sinceDay });
  const out = [];
  for (let i = 0; i < rows.length; i++) {
    const reached = rows[i].reached;
    const next = i + 1 < rows.length && rows[i + 1].depth === rows[i].depth + 1 ? rows[i + 1].reached : 0;
    out.push({
      depth: rows[i].depth,
      reached,
      leftHere: Math.max(0, reached - next),
      leftPct: reached > 0 ? Math.max(0, reached - next) / reached : 0,
    });
  }
  return out;
}
// The versions present, so the admin view can compare releases without being told which exist.
const _depthVersions = db.prepare(
  'SELECT version, SUM(count) AS turns, MAX(depth) AS deepest FROM turn_depth WHERE surface = ? GROUP BY version ORDER BY turns DESC');
export function turnDepthVersions(surface = 'enquiry') { return _depthVersions.all(surface); }
// Median and mean depth REACHED per conversation. A conversation of depth D contributes one turn at each
// of 1..D, so the count of conversations whose length was exactly D is (reached(D) − reached(D+1)) —
// which is `leftHere`. The curve therefore already carries the length distribution.
export function turnDepthSummary(opts = {}) {
  const curve = turnDepthCurve(opts);
  const convos = curve.reduce((s, r) => s + r.leftHere, 0);
  if (!convos) return { conversations: 0, median: 0, mean: 0, turns: 0 };
  const turns = curve.reduce((s, r) => s + r.reached, 0);
  let seen = 0, median = 0;
  for (const r of curve) { seen += r.leftHere; if (median === 0 && seen >= convos / 2) median = r.depth; }
  return { conversations: convos, median, mean: turns / convos, turns };
}
