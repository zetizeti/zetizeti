...
export const APPROACHES = [
  ...
  'ask what happens JUST BEFORE it, or just after',
];
...
function pickAnchor(turns, minReturns, exclude) {
  ...
  let anchor = null, best = 0, bestUses = 0;
  for (const [w, n] of turnsWith) {
    if (n < minReturns || exclude.has(w)) continue;
    const u = uses.get(w) || 0;
    if (n > best || (n === best && u > bestUses)) { anchor = w; best = n; bestUses = u; }
  }
  return anchor ? { anchor, returns: best } : null;
}

export function readDwell({ studentTurns = [], minReturns = 2, legacy = false } = {}) {
  const exclude = new Set();
  let picked = pickAnchor(studentTurns, minReturns, exclude);
  ...
  while (picked) {
    ...
    for (let i = studentTurns.length - 1; i > 0 && held <= ANCHOR_MAX; i--) {
      const prev = pickAnchor(studentTurns.slice(0, i), minReturns, exclude);
    ...
    picked = pickAnchor(studentTurns, minReturns, exclude);
  }
