...
// 🔴 FIRES ONCE PER RUT_TURNS, NOT ON EVERY TURN PAST IT (measured the day it was written). Replaying a
// real learner whose project IS the noun — "trust between strangers" — the invitation to name another part
// came back about trust, because the invitation itself kept the word (it is the subject of the whole
// conversation, and Clean Language says it back), so the run never broke and the reading fired on seven
// consecutive turns. A fixed reply cannot react to an invitation, so a replay overstates this; but seven
// invitations in a row is the tool's fault whatever the learner does. Stateless remedy: fire at exactly
// six, twelve, eighteen — one invitation per six questions on one word, then the ordinary machinery.
/** The longest run of consecutive questions, ending at the latest, that all carry one content word. */
export function readRut(stoneTurns = []) {
  ...
  for (const w of sets[n - 1]) {
    let run = 0;
    for (let i = n - 1; i >= 0 && sets[i].has(w); i--) run++;
    if (run > best.run) best = { word: w, run };
  }
  return best.run >= RUT_TURNS && best.run % RUT_TURNS === 0 ? best : null;
}
