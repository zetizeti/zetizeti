...
// 🔴 A ONE-SITTING WALK GOES DEEP, AND THE TWO-QUESTION BUDGET ABOVE IS NOT ABOUT IT (30 August 2026).
// Prayas, having been handed the whole field in six questions: *"I canno be prepared for 28 projects in
// such a short chat!"* Correct, and the constant was answering a different question. `PREP_DWELL = 2` is
// bounded by the survival curve — people LEAVE, so a student must reach their own enquiry before they go.
// Somebody who has chosen one long sitting has already answered that: they are not going anywhere, and
// the only thing bounding them is their own patience, which they can exercise at any turn by closing the
// tab. Applying a leaving-budget to somebody who has declined to leave costs the whole depth of the arc.
//
// 🔴 AND EARLY ADVANCE IS THE LARGER HALF, WHICH IS NOT OBVIOUS. A station is served once the learner's
// own words have touched a couple of spots in its region — a good rule against grinding, and a disaster
// on somebody FLUENT in the material. He wrote the sheet, so he answers in its vocabulary, so every
// station reads as reached on its first question and the six-station walk finishes in six turns. **Fluency
// and exhaustion are indistinguishable to `regionContact`**, which is the same discriminator problem
// `arc.mjs` records at ANCHOR_MAX: persistence and exhaustion measure identically. So a deep walk does not
// advance early at all; it spends its whole budget on every line.
// ⚠️ EARLY ADVANCE IS SWITCHED OFF BY A FLAG, NEVER BY A HIGH THRESHOLD. `served: Infinity` was the first
// attempt and it is subtly wrong: `need` is `min(served, region.length)`, so an unreachable number becomes
// *the whole region*, which a fluent learner satisfies at once on a SMALL region and not on a large one.
// The walk then behaved differently by document size — 31 turns on a 158-segment sheet, 6 on a short one,
// with nothing to say why. A boolean says what is meant.
//
// 🔴 A DEEP STATION'S BUDGET IS READ OFF ITS REGION, NOT PICKED (Prayas, 30 August 2026: *"I want a
// thorough prep - let it be a few hours if it has to be"*). A single number cannot be right for both ends
// of a real sheet: on the DSL mentor sheet the glossary region holds 42 passages and the worth-and-hype
// region holds 6, so any constant either grinds one line or starves the other. Roughly one question per
// two passages, floored so a thin line still gets a real go and capped so a fat one cannot run away.
//
// 🔴 THE USEFUL CONSEQUENCE IS THAT DEPTH IS A PROPERTY OF THE MATERIAL. A thorough prep is not a setting
// to be turned up; it is what a thorough sheet affords. Somebody who wants a longer sitting writes more
// sheet, and can see in advance roughly what they will get.
export const DEEP_MIN = 4;
export const DEEP_MAX = 24;
export const deepDwellFor = (region = []) =>
  Math.max(DEEP_MIN, Math.min(DEEP_MAX, Math.ceil(region.length / 2)));
export const depthFor = (sittings) => (sittings === 1
  ? { dwell: null, earlyAdvance: false }         // null = read it off each station's own region
  : { dwell: PREP_DWELL, earlyAdvance: true });
...
export function prepPlan({ segments = [], studentTurns = [], stoneTurns = [], tasks = [], sittings = 3 } = {}) {
  ...
  const { dwell: DWELL, earlyAdvance: EARLY } = depthFor(sittings);
  const { stations, anchoredCount } = affordPrep(segments);
  ...
  for (let p = 0; p < parts.length; p++) {
    ...
    for (const si of parts[p]) {
      ...
      let spent = 0;
      const budget = DWELL === null ? deepDwellFor(st.segmentIds) : DWELL;
      while (spent < budget) {
        ...
        if (EARLY && st.anchored && st.segmentIds.length > 0 && touched >= need) break;
      }
    }
