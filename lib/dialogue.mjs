...
${passages}
]${(declined || corrected) ? '' : `${vantageBlock}${aim || ''}${assoc}${widen}${dwellBlock}`}${declined ? '' : successionBlock}${declinedBlock}${correctedBlock}${postureBlock}${shapeBlock}
${message}
...
export function validateOutput(text, { maxWords = 0, avoid = [] } = {}) {
  const reasons = [];
