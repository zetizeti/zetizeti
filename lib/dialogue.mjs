...
export function buildSystemPrompt(methodCore, goal = '') {
  const goalBlock = goal && goal.trim()
    ? `\n== THE LEARNER'S GOAL — the edge they are sharpening ==\nThe learner has named what they are trying to do: "${goal.trim()}"\nOrient every question toward sharpening THIS goal — help them see it more precisely, test the assumptions inside it, and find where it resists them. But do not ONLY press on its weak points: also open where it could go — what it makes possible, what else makes it worth pursuing, an adjacent question it raises. Move the angle each turn — a consequence, a buried assumption, a sideways connection — and never circle the same question you already asked. Each question should be MORE concrete and particular than the one before — reach for a specific tension, moment, consequence, or example; never fall back on a generic opener ("what else could be better", "how might you improve this", "what else could you consider"). When the learner has just sharpened their own thinking, you MAY open with a brief, plain observation of that movement in their own words ("you got more specific just now") — never praise, never evaluation — before the single question. Do not restate or evaluate the goal; question toward it.\n`
    : '';
