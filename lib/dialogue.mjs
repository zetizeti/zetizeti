...
export function buildCriticismSystemPrompt(criticismCore, { artefact = '', located = null, posture = '', retrieved = [], goal = '', focus = null, brief = '', windowNote = '' } = {}) {
  const passages = retrieved.length
...
${criticismCore}
${artefactBlock}${briefBlock}${goalBlock}${focusBlock}${aimBlock}${spot}
...
export function validateCriticismOutput(text, { focus = null, brief = false, artefactTerms = null } = {}) {
  ...
  if (BINARY_DEMAND.test(framing)) {
    reasons.push('offers a choice between two options — ask what is so about the text, not which of two boxes it falls in');
  }
