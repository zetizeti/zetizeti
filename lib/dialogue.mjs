...
export function buildTurnContext({
  ...
  focus = null, prep = null, askingBack = null, rutInvite = null,
}) {
...
${passages}
]${focusBlock}${(declined || corrected || askingBack) ? '' : `${vantageBlock}${aim || ''}${assoc}${widen}${dwellBlock}`}${(declined || askingBack) ? '' : successionBlock}${declinedBlock}${correctedBlock}${askingBackBlock}${(declined || corrected || askingBack) ? '' : `${inviteBlock}${stalledInviteBlock}${rutInviteBlock}`}${askingBack ? '' : `${postureBlock}${precisionBlock}`}${shapeBlock}${openerBlock}${headBlock}
${message}
...
export function validateOutput(text, {
  ...
  noDefine = false, noHypeVerdict = false, mustAddress = false,
} = {}) {
  const reasons = [];
  ...
  reasons.push(...sharedFormChecks(text, stripQuoted(text),
    { maxWords, avoid, banOpeners, banHeads, noBinary, noClosed, ownWords, noCompound, noJargon }));
...
export function buildCriticismSystemPrompt(criticismCore, { artefact = '', located = null, posture = '', retrieved = [], goal = '', focus = null, brief = '', windowNote = '', banOpeners = [], banHeads = [], avoidFrames = [] } = {}) {
  const passages = retrieved.length
...
${criticismCore}
${artefactBlock}${briefBlock}${goalBlock}${focusBlock}${aimBlock}${oneQuestionBlock}${openerBlock}${headBlock}${frameBlock}${spot}
...
function sharedFormChecks(text, framing, { maxWords = 0, avoid = [], banOpeners = [], banHeads = [],
  noBinary = false, noClosed = false, ownWords = null, noCompound = false, noJargon = false } = {}) {
  const reasons = [];
...
export function validateCriticismOutput(text, { focus = null, brief = false, artefactTerms = null,
  ...
  noCompound = false, noJargon = false } = {}) {
  ...
  reasons.push(...sharedFormChecks(text, framing,
    { maxWords, avoid, banOpeners, banHeads, noBinary: true, noClosed, ownWords, noCompound, noJargon }));
