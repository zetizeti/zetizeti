...
export function buildTurnContext({
  ...
  // moves. This is what makes the corpus load-bearing: 265 verified design tensions are 265 places to
  // stand, and until now they only ever informed a question the model was composing about something else.
...
const BINARY_DEMAND = /,\s*or\s+(is|are|does|do|was|were|will|would|the|a|an|to|it|that|something)\b|\bor is it\b|\bor are (you|they)\b|\bor do you\b/i;

export function validateOutput(text, { maxWords = 0, avoid = [], banOpeners = [], mustHold = null, noBinary = false } = {}) {
  const reasons = [];
