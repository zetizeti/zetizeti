...
export function validateOutput(text, {
  ...
  noClosed = false, ownWords = null, focus = null,
} = {}) {
  const reasons = [];
...
export function buildCriticismSystemPrompt(criticismCore, { artefact = '', located = null, posture = '', retrieved = [], goal = '', focus = null, concept = '', windowNote = '' } = {}) {
  const passages = retrieved.length
  ...
  // THE STUDENT'S OWN PROJECT CONCEPT — context for unpacking the text, and NEVER the object of it.
  //
  ...
  const conceptBlock = concept && concept.trim()
    ? `\n== WHAT THE STUDENT IS WORKING ON (an EXTRACT from their own project concept — CONTEXT ONLY) ==\n"""\n${concept.trim()}\n"""\nThis is here so your question about the text can land where the student actually stands: use it to choose WHICH part of the text matters to them and WHICH words to point at. This is an extract, not the whole concept, so never say what their project does or does not contain.\n🔴 THEIR PROJECT IS NOT THE THING UNDER QUESTION. Never ask them to justify, defend, evidence or account for their own concept, and never evaluate it. Every question you ask is about THE TEXT UNDER QUESTION above — asked in the light of what they are working on.\n`
    : '';
...
${criticismCore}
${artefactBlock}${conceptBlock}${goalBlock}${focusBlock}${aimBlock}${spot}
...
export function validateCriticismOutput(text, { focus = null, concept = false, artefactTerms = null } = {}) {
  ...
  if (concept) {
    for (const re of CONCEPT_AS_OBJECT) {
      if (re.test(framing)) {
