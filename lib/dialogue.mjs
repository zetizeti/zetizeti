...
import { jargonIn, jargonReason, JARGON_PROMPT_LINE } from './jargon.mjs';   // no design jargon (v1.10.0)
...
// THE RETURN, told to the composing layer (v1.11.0). One wording for both surfaces, so what the model is asked
// for and what `sharedFormChecks` refuses cannot drift apart. `returnNote` comes from `readReturn` (lib/arc.mjs).
export function returnBlock(returnNote) {
  if (!returnNote) return '';
  const about = returnNote.missing && returnNote.missing.length
    ? ` — about ${returnNote.missing.map((w) => `"${w}"`).join(', ')}`
    : '';
  return `\n[THIS QUESTION GOES BACK TO ${returnNote.subject}, which your last question already asked about, so they will read it as the same question again unless you say why. WHAT THEIR LAST ANSWER DID NOT GIVE: ${returnNote.need}${about}. Name THAT gap and no other. First write ONE short sentence, under ${RETURN_LEAD_MAX} words, ending in a full stop, saying only what their answer has not yet said — do not restate their answer, and do not fill the gap in yourself. This turn only, that sentence comes BEFORE the question, whatever any other rule here says about opening on the question itself or about preambles. Then ask ONLY for that missing part, in a construction your last question did not use, and ask it OPEN — begin it with What, Where, When, Which, Who or How, never "Can you", "Could you" or "Do you", which can be answered yes. Never grade the answer: no "incomplete", "vague", "not clear", "not enough", "good answer". Quote only a whole phrase lifted from your last question; single words need no quotes.]\n`;
}
...
export function buildTurnContext({
  ...
  banOpeners = [], banHeads = [], precision = false, featureInvite = false, stalledInvite = false,
  focus = null, prep = null, askingBack = null, rutInvite = null, returnNote = null,
}) {
  ...
  // On a RETURN the dwell's "come at it a different way" gives way to saying what the last answer left out:
  // the two would pull the one question in two directions, and this file has measured that the more specific
  // instruction wins and the other is work done for nothing.
  const dwellBlock = returnNote ? returnBlock(returnNote) : dwell && dwell.anchor
  ...
  const openerBlock = banOpeners && banOpeners.length
    ...
    : '';
  // THE HEAD, named to the model as well (6 Sep 2026). Naming the openers alone left the model free to
  // keep the template and change the first word, which is what a three-beat metronome is. Quoted, because
  // "do not repeat a construction" is unactionable and "you have begun a question 'Where does the'" is not.
  const headBlock = banHeads && banHeads.length
    ? `\n[Do NOT begin this question with ${banHeads.map((h) => `"${h} …"`).join(' or ')} — recent questions began with those exact words. Change the CONSTRUCTION, not just the noun after it.]\n`
    : '';
...
The particular you ask for must belong to something they have ACTUALLY SAID. Do not ask for the exact moment, point or threshold of a change, shift, transition or trade-off they have not described — a pointed question about an event that never happened is worse than a vague one, because its precision makes the invention sound established.]\n`
  ...
  // THE STALLED INVITE (6 Sep 2026) — they answered, politely, and added not one word they had not
  // already used. Its own block rather than featureInvite's, because that one is written for a learner
  // MAKING something ("another concrete part or feature of what they are making") and this fires on any
  // enquiry, including the 30 August one, where nothing was being made at all. Two things it
  // must not do: press the same material again, which is what produced the stall, and supply candidates,
  // which would make the next answer the tool's rather than theirs.
  const stalledInviteBlock = stalledInvite
    ? `\n[Their last reply added nothing they had not already said — not one new word. Do NOT ask about that material again, and do NOT ask them to expand it. Invite them to name ANOTHER part of what they are looking into — their word for it, no candidates from you — and ask ONE question about whatever they name.]\n`
    : '';

  // THE RUT INVITE (9 September 2026) — the stone's own last questions have held one word for RUT_TURNS
  // running, or (exhausted) every attempt to ask about this material repeated a frame already used. Either
  // way the material is spent from the tool's side, and the one move that cannot re-enter it is to hand the
  // subject back. Same move as the stalled invite, its own reason, and it never names the word to avoid —
  // a word named is a word the model reaches for (nudge.mjs's finding about dictated constructions).
  const rutInviteBlock = rutInvite
    ? `\n[${rutInvite.exhausted
        ? 'Every attempt to ask about the material of the last question repeated the frame of one already asked.'
        : `${rutInvite.run} of the last ${rutInvite.span || rutInvite.run} questions have turned on the same thing.`} Do NOT ask about that material again in any form. Invite them to name ANOTHER part of what they are looking into — their word for it, no candidates from you — and ask ONE question about whatever they name. If they name the same thing again, that is their choice and you go on with it.]\n`
    : '';

  // ASKING BACK (9 September 2026) — they could not follow the last question and said so: "what", "what do
  // you mean by X?", "pls explain", or the tool's own words handed back as a question. This OUTRANKS the
  // aim, the anchor, the succession list and the invites, for the same reason the decline does, and its
  // instruction is the opposite of the decline's: do NOT change the subject. Ask the SAME thing again,
  // plainer, out of words they have used. Nothing is explained and nothing is defined — that is invariant
  // #3 and it is untouched; a question re-asked in plainer words is still a question.
  const askingBackBlock = askingBack
    ? `\n[THEY COULD NOT FOLLOW YOUR LAST QUESTION AND HAVE ASKED WHAT YOU MEANT. Do NOT explain it, do NOT define any word in it, and do NOT change the subject to something else. Ask the SAME thing again in plainer words:
- UNDER FIFTEEN WORDS, one clause.
- Built only from words THEY have used in this conversation${askingBack.words && askingBack.words.length ? ` (for instance: ${askingBack.words.slice(0, 6).map((w) => `"${w}"`).join(', ')})` : ''} — no term of your own, and none of the words from the question they could not follow unless they used them first.
- If the thing you were asking about has no name in their words, ask them what THEY would call it.
- No preamble, no apology beyond a word.]\n`
    : '';
  ...
  // ⚠️ On a RETURN the shape gives way. Every FLOW_SHAPE says "no preamble", and the return needs exactly one
  // sentence before the question — measured on the first live replay (17 Sep 2026): with the shape left in,
  // the model wrote no lead-in in four attempts out of four, because the more specific instruction wins.
  const shapeBlock = shape && shape.trim() && !returnNote
...
- Ask what HAPPENS, or what they NOTICED, or what they would WANT, or simply invite them to say more about the thing they described. Anything they can answer from what they already know.]\n`
  ...
  // THE PREP TURN (28 Aug 2026). The learner brought a sheet about an area they do not work in yet, and this
  // turn is one station of lib/prep.mjs's arc over it. It REPLACES the enquiry steering rather than joining
  // it: vantage, dwell, succession, association, the invite and the precision gate all steer on the
  // learner's own edge, and this turn is not about their edge — it is about a passage in front of both of
  // them. Two directions in one prompt is the failure this file has already named twice (an aim arrives as a
  // direction, a shape as a grammar, and neither displaces the move the model was going to make), so only
  // one direction is given.
  //
  // 🔴 THE DECLINE BLOCK STILL OUTRANKS IT, and that matters more here than anywhere else. Prep runs in an
  // area the learner has just told you they do not know; "I don't know" is the likeliest reply in the whole
  // tool on these turns, and pressing a question about a term they have never met would be the tool's worst
  // available move. The order below leaves declined and corrected exactly where they were.
  // The passage is the material for a station turn and for a closing turn that has to point at what the
  // document suggests. A RESUMING turn has no passage on purpose: its material is the learner's own account,
  // sitting a few turns above in the transcript they brought back, and putting the document in front of the
  // model there would pull the question away from what they actually did and back onto the text.
  const prepPassage = prep && prep.body && prep.phase !== 'resuming'
    ? `\nTHE PASSAGE in front of you, from their own document:\n${prep.body}\n${prep.skeleton ? `(You are seeing part of the document: ${prep.skeleton}. Do not say what the document as a whole does or does not cover.)\n` : ''}`
    : '';
  // The task, verbatim, from whoever prepared their material. Quoted rather than paraphrased into the aim,
  // because a task the stone rewrote is a task the stone set.
  const prepTask = prep && prep.task
    ? `\nTHE TASK ALREADY SET for the gap before the next part, written by the person who prepared their material — quote or state it, never reword or replace it:\n"${prep.task}"\n`
    : '';
  const prepBlock = prep && prep.aim
    ? `\n[THIS IS A PREPARATION TURN — part ${prep.part || 1} of ${prep.parts || 3}. The learner brought a document about an area they do not work in yet, and you are walking them into it BY ASKING — never by explaining. They have the document; it does the telling. Your whole job is to find out where they stand in it.
${prepPassage}${prepTask}
${(declined || corrected || askingBack) ? '' : `THIS TURN'S LINE: ${prep.aim}\n`}
Do NOT define, gloss, explain, summarise or correct any term. Do NOT tell them what the field thinks. You may QUOTE the document's own words — they have it open — but nothing may be asserted in your own voice.

ASK ABOUT THEM, NEVER ABOUT THE FIELD IN GENERAL. The question must be one that only this person can answer: what they already took a word to mean, what they can already do, what they have around them this week, what they would want to see before believing something. A question with no "you" in it is a question any model anywhere can answer, and it will be withheld. Nothing they say is marked, scored or checked against the document — you are finding out where they stand, not testing them. One question, in their words and the document's.]\n`
    : '';

  // 🔴 A DECLINE SUPPRESSES THE STATION'S LINE, not just the enquiry steering above. The decline block's
  // instruction is to change footing and ask something EASIER; leaving the station's aim in beside it would
  // hand the model two incompatible orders and it would follow the more specific one, which is the aim —
  // asking the same hard thing about the same passage in different words. That is exactly what the decline
  // block exists to prevent, and it is why the aim is dropped inside the block above rather than after it.
  if (prepBlock) {
    return `${prepBlock}${declinedBlock}${correctedBlock}${askingBackBlock}${shapeBlock}${openerBlock}${headBlock}
${message}

[Reply with ONE short question only — a single sentence. No second question, no follow-up. Ask; never tell.]`;
  }

  return `[DOMAIN MATERIAL for this turn — tensions in my discipline, for you to sharpen your question, not to state back to me:
${passages}
]${focusBlock}${(declined || corrected || askingBack) ? '' : `${vantageBlock}${aim || ''}${assoc}${widen}${dwellBlock}`}${(declined || askingBack) ? '' : successionBlock}${declinedBlock}${correctedBlock}${askingBackBlock}${(declined || corrected || askingBack) ? '' : `${inviteBlock}${stalledInviteBlock}${rutInviteBlock}`}${askingBack ? '' : `${returnNote ? '' : postureBlock}${precisionBlock}`}${shapeBlock}${openerBlock}${headBlock}
${message}

${returnNote
    // The closing line is the last thing the model reads and the one it follows. On a return it must ask for
    // the lead-in, or it contradicts the block above and the guard refuses every attempt.
    ? '[Reply with ONE short plain sentence that says what their last answer did not yet say, ending in a full stop, then ONE short question asking only for that. No second question.]'
...
// ── THE METRONOME, and the two windows that set its period (6 September 2026) ────────────────────
// A real AI Club session of 30 August ran eighteen questions whose opening words were, in order:
//   what how where  what how where  what how where  what how where  what how where  what how where
// Six laps of a three-beat cycle, unbroken, and NOT ONE GUARD FIRED. Each did exactly what it was
// written to do:
//   · the OPENER BAN is two questions wide (`stoneTurns.slice(-2)`), so it forbids every cycle of
//     length 1 or 2 and PERMITS a cycle of length 3. It does not bound repetition; it sets a FLOOR
//     under the period, and the model sat on the floor. A guard whose satisfying set has a cheapest
//     member will be met by that member — this is the first time that has been visible here.
//   · the FRAME GATE compares FIVE-word shingles, and the template repeating was three words long
//     ("Where does the …" ×6, "What happens to the … when …" ×3). The comment above `avoid` records
//     the identical blindness one size up, at four-word prefixes, on 29 July; the resolution was
//     raised and the blind spot moved rather than closing.
// So HEAD is the missing reading: the first three words of the question sentence, banned across a
// WIDER window than the opener. Three words is where a template lives — long enough to be a frame,
// short enough that swapping the noun does not escape it.
// 🔴 ONE DERIVATION FEEDS THE PROMPT AND THE GUARD (`openerBans` / `headBans` below), because two
// derivations of "what the last questions opened with" would drift, and the composing layer must be
// TOLD what will be refused or the guard is a repair loop (17 August).
export const OPENER_BAN_TURNS = 3;   // was 2 until 6 Sep 2026: minimum legal opener cycle is now 4
export const HEAD_BAN_TURNS = 6;     // the head is banned twice as far back as the opener
export const HEAD_WORDS = 3;
// The first HEAD_WORDS words of the question sentence, lowercased. '' when the sentence is shorter
// than that — a two-word question has no template to repeat. Quoted spans are NOT stripped here: a
// question that opens by quoting the learner opens with their words, and this is about the frame the
// model reaches for, which is the same object either way.
export function questionHead(text, n = HEAD_WORDS) {
  const m = String(text).match(/(?:^|[.!]\s+)([A-Za-z"“'][^.!?]*\?)/);
  if (!m) return '';
  const w = m[1].toLowerCase().replace(/[^a-z0-9' ]/g, ' ').split(/\s+/).filter(Boolean);
  return w.length >= n ? w.slice(0, n).join(' ') : '';
}
export const openerBans = (stoneTurns = [], turns = OPENER_BAN_TURNS) =>
  [...new Set(stoneTurns.slice(-turns).map((q) => questionOpener(q)).filter(Boolean))];
export const headBans = (stoneTurns = [], turns = HEAD_BAN_TURNS) =>
  [...new Set(stoneTurns.slice(-turns).map((q) => questionHead(q)).filter(Boolean))];
...
const PREAMBLE_TELLS = [
  ...
  // APPRAISAL (9 September 2026). Eight preambles in twenty real dialogues valued the learner's material —
  // "is doing real work" ×3, "is opening something" ×3, "is becoming clearer", "is opening the way you
  // see" — and every one was the system prompt's own example sentence said back, because the prompt and
  // the warmth posture both instructed it. The instruction is gone from both; these refuse the shape if it
  // comes back on its own. A verdict on what somebody's material is DOING is a judgement on the material,
  // one step from a judgement on them (#7), and it is theirs to make. `forms: []` throughout — the learner
  // having said "work" does not license "is doing real work".
  { re: /\bdoing\s+(?:real|good|useful|important|serious|the|some)\s+work\b/i,                     forms: [] },
  { re: /\b(?:is|are|'s)\s+(?:becoming|getting|growing)\s+(?:clearer|sharper|stronger|richer)\b/i, forms: [] },
  { re: /\bopening\s+(?:something|up|the\s+way|a\s+(?:door|space|line|path))\b/i,                forms: [] },
  { re: /\b(?:is|are|'s)\s+(?:a\s+)?(?:strong|useful|productive|sharp|important|good|precise|clear|interesting|rich|powerful|helpful|nice|great|key|crucial|real)\s+(?:distinction|move|point|line|thread|start|question|observation|idea|shift|step|one)\b/i, forms: [] },
  { re: /\breal\s+tension\s+is\s+(?:now\s+)?in\s+view\b/i,                                    forms: [] },
  { re: /\bworth\s+(?:holding|noting|pausing|staying)\b/i,                                       forms: [] },
  { re: /\bstrikes?\s+me\b/i,                                                                   forms: [] },
];
...
// ───────────────────────── the prep guards (28 August 2026) ─────────────────────────
// Both are ENQUIRY-ONLY and both are listed here rather than left to be inferred, because this file's own rule is that a difference between the two surfaces is either chosen or it is a defect. They are chosen: a prep turn is the only place in this tool where the model is handed a document full of definitions and asked to question a learner who does not yet know the words. Nothing on the criticism surface is in that position — there the learner brought a text they are already suspicious of.
//
// 🔴 noDefine — THE TOOL MAY NAME A TERM AND MAY NEVER SAY WHAT IT MEANS. That one line is the difference between prep and teaching, and prep is the only feature here whose brief actively pulls toward the wrong side of it. FORBIDDEN already catches the blunt forms ("the answer is", "you should"); a definition is fluent, helpful, and carries none of them — the same shape as the invented premise, which passed every rule aimed at clumsiness.
// ⚠️ Checked on the QUESTION only, after the preamble, with quotes stripped — the two exemptions that make this safe rather than merely strict. The warmth clause says the learner's own words back and invariant #1 requires it; and QUOTING their own document is the method, not an assertion, because they have the file open in front of them. What is refused is the tool glosing a term in its own voice.
const DEFINES = [
  /\brefers to\b/i,
  ...
  /\bi\.e\.\b/i,
  /\bthat is to say\b/i,
  /\bwhich (?:is|are)\s+(?:a|an|the)\b/i,
  /\b(?:is|are)\s+when\b/i,
  /\b(?:is|are)\s+(?:basically|essentially|simply|really just)\b/i,
  /\bmeans\s+(?:that\s+)?(?:a|an|the|to|when|it|you|we|they)\b/i,
  /\b(?:is|are)\s+(?:a|an|the)\s+[a-z-]+(?:\s+[a-z-]+)?\s+(?:that|which|used|for|by)\b/i,
];
// 🔴 noHypeVerdict — THE STONE ASKS WHAT IS HYPE AND NEVER SAYS. The worth-and-hype station exists because Prayas asked for it, and it is the one station whose subject IS a judgement — which makes it the one place where a question can smuggle its own answer and read as perfectly ordinary. "What makes this overhyped claim so appealing?" is not a question about whether it is hype. Invariant #5 is what this enforces, on the only turn that can breach it.
// ⚠️ A word the LEARNER used is theirs and passes, the same acquittal PREAMBLE_TELLS grants — if they have called something hype, saying it back is Clean Language, not the tool deciding.
// 🔴 THE ACQUITTAL IS PER WORD, NOT PER PATTERN, and the first version was not — it listed every word a
// pattern could match and waved the pattern through if the learner had used ANY of them. So a student who
// had once said "hype" licensed the stone to call something "just marketing", which is a verdict they never
// made being delivered under cover of a word they did. Found by planting the sentence rather than by reading
// the rule. Each entry now captures the word it actually matched and only THAT word can be theirs.
const HYPE_VERDICT = [
  { re: /\b(overhyped|overrated|underrated)\b/i, acquittable: true },
  { re: /\b(hyped)[- ]up\b/i, acquittable: true },
  { re: /\b(?:is|are|remains|remain|stays|was|were)\s+(?:just\s+|merely\s+|only\s+|mostly\s+|largely\s+|all\s+)?(?:a\s+|an\s+)?\b(hype|fad|bubble|marketing|noise)\b/i, acquittable: true },
  // No acquittal: "nothing but" carries the dismissal in the construction, not in a word they could have used.
  { re: /\bnothing but\b/i, acquittable: false },
];

// 🔴 mustAddress — A PREP QUESTION IS ADDRESSED TO THE LEARNER, and this is the structural half of the
// answer to "they will just ask another model" (Prayas, 28 August 2026). A question with no second person
// in it is a question about the FIELD, and a question about the field has an answer sitting in every model
// on earth; a question about where this person stands does not. Requiring "you" or "your" outside quoted
// spans is the cheapest true statement of that, and it is a property of the SENTENCE — nothing here reads
// the learner, which is what keeps it the right side of invariant #7.
// ⚠️ It costs the impersonal phrasings, and one of them was good: "what would have to be true for that to
// hold?" is refused and has to become "what would you need to see before you believed that?". That is a
// real loss of variety, accepted deliberately, because the impersonal form is precisely the one that can be
// pasted into another window and answered.
const ADDRESSES = /\b(?:you|your|yours|yourself)\b/i;

export function validateOutput(text, {
  maxWords = 0, avoid = [], banOpeners = [], banHeads = [], mustHold = null, noBinary = false,
  noClosed = false, ownWords = null, focus = null, noCompound = false, noJargon = false,
  noDefine = false, noHypeVerdict = false, mustAddress = false, returnNote = null,
} = {}) {
  const reasons = [];
  ...
  // THE PREP GUARDS. Both read the QUESTION rather than the whole turn, for the reason the concept filter
  // already established one block above: the preamble legitimately says the learner's own words back, and
  // checking it refused turns whose question was fine. Quotes are stripped, so a span lifted from the
  // learner's own prep sheet is never mistaken for the tool asserting something.
  if (noDefine || noHypeVerdict || mustAddress) {
    const pre = preambleOf(text);
    const at = pre ? String(text).indexOf(pre) : -1;
    const asked = stripQuoted(at >= 0 ? String(text).slice(at + pre.length) : String(text));
    if (mustAddress && !ADDRESSES.test(asked)) {
      reasons.push('the question asks about the field rather than about them — any model can answer that one, and this is not the tool for it; ask what THEY already took it to mean, what they can already do, what they would try first, or what they would want to see before believing it');
    }
    if (noDefine) {
      for (const re of DEFINES) {
        if (re.test(asked)) {
          reasons.push('the question tells them what a term means instead of asking — they brought the document, so it does the explaining; quote their word and ask what they already take it to mean, or where they have met it');
          break;
        }
      }
    }
    if (noHypeVerdict) {
      for (const t of HYPE_VERDICT) {
        const m = asked.match(t.re);
        if (!m) continue;
        if (t.acquittable && m[1] && ownWords instanceof Set && ownWords.has(m[1].toLowerCase())) continue;
        reasons.push('the question has already decided whether this claim is worth anything — that judgement is theirs and asking for it is the point of the turn; quote the claim plainly and ask what they make of it, or what they would want to see before believing it');
        break;
      }
    }
  }
  ...
  reasons.push(...sharedFormChecks(text, stripQuoted(text),
    { maxWords, avoid, banOpeners, banHeads, noBinary, noClosed, ownWords, noCompound, noJargon, returnNote }));
...
export function buildCriticismSystemPrompt(criticismCore, { artefact = '', located = null, posture = '', retrieved = [], goal = '', focus = null, brief = '', windowNote = '', banOpeners = [], banHeads = [], avoidFrames = [], returnNote = null } = {}) {
  const passages = retrieved.length
...
${criticismCore}
${artefactBlock}${briefBlock}${goalBlock}${focusBlock}${aimBlock}${returnBlock(returnNote)}${oneQuestionBlock}${openerBlock}${headBlock}${frameBlock}${spot}
...
// A grade on the learner's answer, in the clause that says what it left out. Naming the gap is a fact about two
// texts; calling the answer incomplete is a verdict on somebody's material, which is theirs (the 9 September
// reversal of the warmth dial). Asking for a detail is allowed — "not detailed enough" is not.
const ANSWER_ABSENCE = /\b(?:not|no|never|nothing|without|yet|unsaid|unanswered|unstated|undefined|unnamed|missing|left\s+out)\b|n['’]t\b/i;
// The plain words a return's lead-in needs to talk about an answer at all. Licensed on a return turn by both
// routes, so the lead-in rule (the clause may only say back the learner's words) does not refuse
// "your answer did not mention …" for using "answer" and "mention".
export const RETURN_LEAD_WORDS = ['answer', 'answered', 'answers', 'mention', 'mentioned', 'said', 'say', 'yet',
  'describe', 'described', 'part', 'detail', 'details', 'still', 'last', 'question', 'told', 'tell', 'reply', 'wrote',
  // naming the absence, and naming the KIND of detail that is absent — measured live: "The specific moment of
  // that shift remains unstated." named the right gap and was refused for "moment", "remains", "unstated"
  'remain', 'remains', 'unstated', 'unsaid', 'undefined', 'unanswered', 'missing', 'specific', 'exactly', 'moment',
  'point', 'time', 'when', 'place', 'where', 'reason', 'why', 'person', 'people', 'who', 'which', 'many', 'much',
  'identified', 'located', 'defined', 'shown', 'named', 'name', 'explained', 'happens', 'true'];
export const RETURN_LEAD_MAX = 22;
const ANSWER_VERDICT = /\b(?:incomplete|vague|unclear|insufficient|partial(?:ly)?|superficial|shallow|not\s+(?:yet\s+|very\s+|fully\s+)?(?:enough|clear|specific|detailed|complete|quite)|too\s+(?:short|brief|general|vague|broad)|(?:good|great|nice|interesting|strong|fair)\s+(?:answer|start|point|reply)|did\s*n[o']?t\s+(?:really\s+|quite\s+|fully\s+)?answer|missed\s+the\s+(?:question|point))\b/i;
function sharedFormChecks(text, framing, { maxWords = 0, avoid = [], banOpeners = [], banHeads = [],
  noBinary = false, noClosed = false, ownWords = null, noCompound = false, noJargon = false, returnNote = null } = {}) {
  const reasons = [];
  // THE RETURN SAYS WHY (v1.11.0, 17 Sep 2026 — Prayas: "when a question is repeated (seemingly) because the
  // student's answer was neither complete or detailed enough - specify what was left out, what detail is
  // still needed"). Code has already decided the coming question returns to the last one's subject
  // (`readReturn`, lib/arc.mjs). Telling the model so is a request; this is the refusal. The question must
  // open with a clause that NAMES what the answer left out — a word of the last question — and that clause
  // may not grade the answer. Shared, so the two surfaces that return (enquiry and critique) hold it alike.
  if (returnNote && Array.isArray(returnNote.words) && returnNote.words.length) {
    const pre = preambleOf(text);
    if (!pre) {
      reasons.push('this question goes back to what your last question asked about, so it will read as the same question again — first say, in one short sentence ending in a full stop, what their last answer did not yet say, then ask only for that');
    } else {
      const said = String(pre).toLowerCase().match(/[a-z]{3,}/g) || [];
      const named = returnNote.words.some((w) => said.some((x) => x === w || (w.length >= 5 && x.length >= 5 && x.slice(0, 5) === w.slice(0, 5))));
      if (!named) reasons.push(`the sentence before the question must name what their last answer left out, using a word from your last question (${returnNote.words.slice(0, 4).join('/')})`);
      if (ANSWER_VERDICT.test(stripQuoted(pre))) {
        reasons.push('the sentence before the question grades their answer — say what it did not yet say, never how good, full or clear it was');
      }
      // 🔴 IT MUST SAY WHAT IS ABSENT, not fill the absence in. Measured on the first live run with the lead-in
      // working: "The roof protects the tools from the rain." names the right words and states, as fact, the
      // answer the learner had not given — the never-answer invariant broken one sentence early.
      if (!ANSWER_ABSENCE.test(stripQuoted(pre))) {
        reasons.push('the sentence before the question states something instead of saying what their answer did NOT yet say — never fill the gap in for them; say what is still missing');
      }
      // And the question then asks for the missing part, not for something new.
      const target = Array.isArray(returnNote.ask) && returnNote.ask.length ? returnNote.ask : returnNote.words;
      const asked = String(text).slice(String(text).indexOf(pre) + pre.length).toLowerCase().match(/[a-z]{3,}/g) || [];
      if (!target.some((w) => asked.some((x) => x === w || (w.length >= 5 && x.length >= 5 && x.slice(0, 5) === w.slice(0, 5))))) {
        reasons.push(`the question moves on instead of asking for what was missing — ask for that part (${target.slice(0, 4).join('/')})`);
      }
      // The lead-in is one short sentence. 22 is FLOW_SHAPES' own ceiling for a sentence here.
      const leadWords = pre.split(/\s+/).filter(Boolean).length;
      if (leadWords > RETURN_LEAD_MAX) reasons.push(`the sentence before the question is too long: ${leadWords} words (max ${RETURN_LEAD_MAX}) — say what is missing in fewer`);
    }
  }
  // NO DESIGN JARGON (v1.10.0, 16 Sep 2026 — Prayas: "make sure neither the questions, nor the explanations
  // use design jargon"). Read on `framing`, so a span quoted from the text or from the learner passes, and a
  // term the learner used themselves is theirs (lib/jargon.mjs). Shared, so it reaches all three surfaces.
  if (noJargon) {
    const found = jargonIn(framing, ownWords instanceof Set ? ownWords : null);
    if (found.length) reasons.push(jargonReason(found));
  }
  if (noCompound) {
  ...
  if (maxWords > 0) {
    // On a RETURN the cap reads the question alone; the lead-in has its own ceiling (RETURN_LEAD_MAX) above.
    const counted = returnNote && preambleOf(text) ? String(text).slice(String(text).indexOf(preambleOf(text)) + preambleOf(text).length) : text;
    const n = plainQuestion(counted).trim().split(/\s+/).filter(Boolean).length;
  ...
  // THE HEAD BAN (6 Sep 2026) — the three-word template, banned across a wider window than the opener.
  // Sits BESIDE the opener ban rather than replacing it: the opener stops "where … where", the head stops
  // "Where does the … Where does the" with a different noun in between, which is what six laps of a
  // three-beat cycle actually looked like. Reported with the head quoted, because "vary the construction"
  // is unactionable and "you have already begun three questions 'Where does the'" is not.
  if (banHeads && banHeads.length) {
    const hd = questionHead(text);
    if (hd && banHeads.includes(hd)) {
      reasons.push(`the question begins "${hd} …" again — a recent question began with those same three `
        + 'words; change the construction, not just the noun after it');
    }
  }
...
export function validateCriticismOutput(text, { focus = null, brief = false, artefactTerms = null,
  maxWords = 0, avoid = [], banOpeners = [], banHeads = [], noClosed = false, ownWords = null,
  noCompound = false, noJargon = false, returnNote = null } = {}) {
  ...
  reasons.push(...sharedFormChecks(text, framing,
    { maxWords, avoid, banOpeners, banHeads, noBinary: true, noClosed, ownWords, noCompound, noJargon, returnNote }));
...
// ───────────────────────────── the prep arc (28 August 2026) ─────────────────────────────
// The aims for lib/prep.mjs's six stations. They live HERE, with the rest of the prompt wording, exactly as CRITICISM_POINTERS does, and prep.mjs checks its rule list against this one at module load so the two cannot drift silently.
//
// 🔴 EVERY ONE OF THESE ASKS AND NONE OF THEM TELLS, and that is not a style note. "Prep the student for a new area" is the one brief in this whole tool that reads like a licence to explain, and it is not one: the learner brought the document, so the document does the telling. What the stone does is point at a passage of it and ask what they already hear there. `noDefine` refuses the version of this that goes wrong.
//
// 🔴 AND THE LAST ONE NEVER SAYS WHAT IS HYPE. The station points at a sentence in which the field makes a large claim about itself, and asks the learner what they make of it. Invariant #5 puts that judgement on their side of the line permanently. A question that arrives already knowing which claims are noise has answered itself, and `noHypeVerdict` withholds it.
// 🔴 EVERY AIM ASKS FOR THE LEARNER'S OWN POSITION, AND THAT IS A DESIGN AGAINST OUTSOURCING RATHER THAN A
// STYLE PREFERENCE (Prayas, 28 August 2026: "they can use some other ai to get answers and they most
// probably will - so work around that"). He is right that they will, and the tempting answer — read their
// replies for signs of a model having written them — is the wrong genus of fix twice over. It would be a
// judgement about the LEARNER, which invariant #7 keeps out of this system entirely, and it would be built
// from a phrase list, which this file already holds can only ever find students who write like the one it
// was written from.
//
// So the fix is upstream of the cheating: ask questions whose answers do not exist anywhere but in the
// person answering. What THEY already took a word to mean before today; what they can already do and what
// they would have to go and learn; what is in their studio this week; what they personally would need to
// see before they believed a claim; which of these they intend to ignore. Another model will compose a
// fluent answer to any of these and it will be worth nothing, because there is no mark to collect and
// because the next question asks about the position the last answer just claimed — which is expensive to
// keep faking and pointless to start.
//
// ⚠️ NONE OF THESE AIMS MAY RECOMMEND A PHRASE THE GUARD REFUSES, and the first draft of the last one did.
// It asked for "what would you need to see before you believed it", which trips FORBIDDEN's `you need to` —
// a pattern that has guarded the never-answer invariant since the beginning and exists to catch advice.
// Every worth-hype turn would have been generated, refused and regenerated. It was found by running the
// guard against the aims rather than by reading either, which is the case for the probes in one line. The
// fix went into the AIM and never into the gate: loosening an invariant's pattern to make a new feature
// comfortable buys the number by giving up the thing being measured.
//
// ⚠️ The corollary is that a well-formed prep question is ADDRESSED to them, and `mustAddress` enforces
// exactly that and nothing more: the question must contain "you" or "your" outside anything quoted. It is a
// structural property of the sentence, not a reading of the student — the distinction the whole surface
// rests on. "What does latency mean in embedded systems?" fails it, and should: any model can answer that,
// and this tool was never for asking it.
export const PREP_POINTERS = [
  { key: 'glossary',      aim: "Quote ONE term from the passage — their document's own word — and ask what THEY already took it to mean before today, or where they have met it before, or what they first assumed when they read it. Do NOT tell them what it means, do not gloss it, do not check whether they got it right. You are finding out what the word already does for them, which is a thing only they know." },
  { key: 'background',    aim: "The passage says where something came from. Ask what that history changes for THEM — what it makes them expect, what they had assumed instead, or which part of it they had not known. Never supply the history and never correct theirs; ask what it does to their picture of the field." },
  { key: 'prerequisites', aim: "The passage names something that must be in place first. Ask what they can ALREADY do of it and what they would have to go and learn — where they actually stand, in their own account. Do not list what they need." },
  { key: 'long-shots',    aim: "The passage marks something as far off, unsettled or expensive. Ask what THEY would need — time, money, a skill, a collaborator — before they could go near it, or which part of it they would want somebody else to solve first. Do not answer it for them and do not soften it." },
  { key: 'low-hanging',   aim: "The passage marks something as already to hand. Ask what they could make with it THIS WEEK, with what they already have around them, or which one they would pick up first and why that one. Concrete, small, and theirs." },
  { key: 'worth-hype',    aim: "The passage makes a large claim about this field. Quote the claim and ask what THEY would want to see before believing it, or what they would expect to have changed in a year if it were true, or whether they would spend their own time on it. NEVER say, imply, or assume whether the claim is hype: that judgement is theirs, and asking them for it is the whole point of this turn." },
];

// THE TURN THAT CLOSES A PART (28 August 2026). The learner is about to go away and come back, and what
// makes the gap worth anything is that something happens in it.
// 🔴 THE STONE DOES NOT SET THE TASK. Where the sheet lists things to try, quote one or two of its own words
// and ask which they will take; where it lists none, ask them to name something small themselves. Either
// way the choosing is theirs. "Go and build a small one" is an instruction, and this tool does not give
// them — the difference between pointing at a suggestion somebody else wrote and issuing it is the whole
// distance between asking and answering.
// 🔴 WHEN A TASK HAS BEEN SET, THE STONE DELIVERS IT AND QUESTIONS IT — it does not compose it, soften it,
// improve it or add a second one. The task is somebody else's decision, arriving verbatim, and the only
// thing the stone adds is one question about how this particular person will meet it. That question is
// where the value is: a task read and a task thought about are different things, and only the second
// survives a week away.
export const PREP_CLOSING_AIM_SET = "This part is ending and they are about to go away and come back. The task for the gap has ALREADY BEEN SET by the person who prepared their material, and is quoted above. State it back to them plainly in one short clause — do not reword it into something easier, do not add a second task, do not offer alternatives — and then ask ONE question about how THEY will meet it: what they will do first, what they already have for it, or what they expect to get stuck on.";
export const PREP_CLOSING_AIM = "This part is ending and they are about to go away and come back. Nobody has set a task, so ask what ONE small thing THEY will go and do before then — pointing at what their own document suggests trying, if it suggests anything. Small, concrete, and this week. Do NOT tell them what to do, do not propose a task of your own, and do not add encouragement.";

// THE TURN THAT OPENS A LATER PART. Everything it needs is in the transcript the learner brought back: what
// they said they would do is sitting in their own words a few turns above.
// ⚠️ IT MUST NOT ASSUME THEY DID IT. A question shaped "how did it go when you built it?" presupposes the
// building, and a learner who did nothing then has to argue with the premise before they can answer — the
// invented-premise fault, arriving where it would land hardest, on somebody returning after a week away.
export const PREP_RESUMING_AIM = "They have come back after a gap. Earlier in this conversation they said what they would go and do. Ask what actually happened with it — what they did, what they could not do, or what they found instead. Take their own words for it from the transcript. Do NOT assume they did it, do not congratulate, do not scold, and do not recap what was covered before.";
...
// ---------------------------------------------------------------------------------------------------
// THE SPECCING SURFACE. A student's own SPECIFICATION is the object; the eight lines in
// `lib/spec.mjs` are the lines of questioning.
//
// 🔴 WHY IT DOES NOT REUSE `buildCriticismSystemPrompt` OR `validateCriticismOutput`. Criticism's whole
// polarity is that the found text is the object and the student's project is CONTEXT, and it enforces
// that with SELF_DEFENCE below — a refusal of any question pairing "your project" with "justify /
// defend / prove". Here the student's document IS the object, so that refusal would refuse the surface's
// own purpose. Reusing either function and carving out an exception would leave criticism's rule saying
// something it no longer means. Two functions; one set of shared FORM checks.
// ---------------------------------------------------------------------------------------------------

// The verdict register this surface must not slip into. Criticism has its own list aimed at pronouncing
// on a text; this one is aimed at the specific failure of a spec-reader, which is COMPLETING THE SPEC.
// Every one of these is a way of supplying the content the student was supposed to supply.
const SPEC_FORBIDDEN = [
  // "you have not said / your spec does not mention / this is missing" — true, and an answer. The absence
  // is the student's to find; the question goes to the place, not to the gap.
  /\b(you (have not|haven'?t|did not|didn'?t)|your (spec|specification|document|description)( does not| doesn'?t| never))\b/i,
  /\b(is|are|it'?s) (missing|absent|unspecified|undefined|not (specified|defined|mentioned|covered))\b/i,
  /\bthere is no mention\b/i,
  // "you should / you need to / consider adding / don't forget" — instruction wearing a question mark.
  /\b(you (should|need to|ought to|must)|consider (adding|specifying|defining)|don'?t forget|remember to)\b/i,
  // "have you considered X" — the commonest way a reviewer smuggles the answer in as a question.
  /\bhave you (considered|thought about|accounted for|handled)\b/i,
  // Verdicts on the spec as a piece of work. It is unfinished on purpose; grading it is a different tool.
  /\b(this (spec|specification) is|your (spec|specification) is) (good|bad|weak|strong|thorough|vague|incomplete|complete)\b/i,
];

// 🔴 A QUESTION MAY ONLY POINT WITH WORDS ALREADY ON THE PAGE, and on this surface that check is the
// whole defence rather than a refinement of it. Inventing a noun — "what happens at the corner?" when
// she never wrote "corner" — is how a question becomes a suggestion: it names the thing she left out,
// which is the answer. `inventionReasons` already does this on the enquiry surface; here it is not
// optional and the route always passes `ownWords`.

export function buildSpecSystemPrompt({ spec = '', assignment = '', joint = null, asked = [],
  banOpeners = [], banHeads = [], avoidFrames = [], circling = [] } = {}) {
  const specBlock = `\n== THE SPECIFICATION UNDER QUESTION (the student's own, and it is UNFINISHED ON PURPOSE) ==\n"""\n${String(spec).trim()}\n"""\n`;

  // The assignment, when there is one, is what was ASKED FOR — never a rubric to mark against.
  // ⚠️ The risk it carries is the opposite of criticism's: there, the student's work could wrongly become
  // the object; here, the assignment could wrongly become a standard the stone enforces. It is context
  // for choosing WHERE to ask, and the instruction says so.
  const assignmentBlock = assignment && assignment.trim()
    ? `\n== WHAT WAS ASKED FOR (context only — NOT a rubric, and you never mark against it) ==\n"""\n${assignment.trim()}\n"""\nUse this only to choose which part of their specification matters. Never say whether they have met it, never list what it requires, and never ask them to justify their specification against it.\n`
    : '';

  // 🔴 THE COUNTER-EXAMPLE IS CARRIED WHERE THERE IS ONE, and it is not decoration. Measured 3 September
  // 2026: `refusal` produced nought questions of three that were about refusing, and `thing` two of six.
  // Both failed the same way — the model reached for the NEAREST ordinary question and labelled it with
  // the joint it had been given. An aim states the target; only a wrong answer states the near-miss, and
  // the near-miss is what it was actually writing.
  const exampleBlock = joint && joint.example ? `${joint.example}\n` : '';
  // 🔴 THE DISTINCTION `refusal` NEEDS, AND ITS ABSENCE IS WHY THAT JOINT INVERTED. This surface forbids
  // naming what her DOCUMENT does not say, and the refusal line asks what her OBJECT does not know. Those
  // are different things and the prompt never said so, so a question about the object's ignorance read to
  // the model as the refusal it had just been handed, and it wrote the opposite question instead — "how
  // does the shelf show that a tool is borrowed?" three times out of three. Stated only on that joint,
  // because on the other seven there is nothing to disambiguate and a general caveat would soften them.
  const notThisBlock = joint && joint.key === 'refusal'
    ? '🔴 A thing her OBJECT does not know is not the same as something her DOCUMENT has not said. '
      + 'Asking what the object declines to know is this line\'s whole purpose and is never "naming an '
      + 'absence" — that rule is about her writing, not about her design. Ask it directly.\n'
      + '🔴 THE QUESTION IS ABOUT WHAT IS NOT KNOWN. A question about what it DOES know, however '
      + 'carefully phrased, is the opposite of this line. "How does the box distinguish somebody walking '
      + 'in from somebody walking out?" asks what it works out; this line asks what it does not.\n'
      + '🔴 BUT DO NOT INVENT A REFUSAL EITHER. "Why does the window refuse to record the bounces?" and '
      + '"how does the shelf refuse to record how long a tool has been away?" both assert a refusal she '
      + 'never wrote and then ask her to explain it — the presupposition fault, arriving through this '
      + 'line. Ask from OUTSIDE the thing instead: "what could somebody not find out from the shelf, '
      + 'however long they stood there?" asserts nothing about how it works.\n'
    : '';
  // 🔴 THE CLOSING CLAUSE IS JOINT-AWARE, AND ITS ABSENCE IS MOST OF WHY THE TWO NEW LINES FAILED.
  // It read "ask what theirs DOES at that place" for every line — which is right for the six joints and
  // is the opposite instruction on both new ones: `thing` asks what it IS and `refusal` asks what it does
  // NOT know. It also sat LAST in the block, after the aim and the counter-example, so the contradiction
  // was the most recent thing the model had been told. Measured 3 September 2026, before this: `refusal`
  // nought of three, `thing` two of six.
  const closer = joint && (joint.key === 'refusal' || joint.key === 'thing')
    ? 'ask it anyway and let them discover the gap by trying to answer.'
    : 'ask what theirs does at that place and let them discover the gap by trying to answer.';
  const jointBlock = joint
    ? `\n== THE LINE OF QUESTIONING FOR THIS TURN — ${joint.label} ==\n${joint.aim}\n${exampleBlock}${notThisBlock}🔴 Ask about THIS, in ONE question, using their own words. If their specification says nothing about it, do NOT say so and do NOT name what is missing — ${closer}\n`
    : '';

  // 🔴 AND THE AIM IS RESTATED AT THE VERY END. A general rule stated later in a prompt outranks a
  // specific one stated earlier, which is what was happening: HOW TO ASK enumerated does/holds/decides/
  // shows and the two new lines are neither, so the model followed the global list and labelled the
  // result with the line it had been given. One sentence, naming the line, immediately before the
  // instruction to write.
  const tailBlock = joint
    ? `\n🔴 THIS QUESTION IS ABOUT ONE THING ONLY: ${joint.label}. ${joint.aim}\n`
    : '';

  const spentBlock = asked && asked.length
    ? `\n[Already asked about: ${asked.join(', ')}. Do not return to those.]\n`
    : '';

  // 🔴 ROTATING THE JOINT DOES NOT MOVE THE SUBJECT, and a ten-round run proved it: all six joints were
  // visited, longest run on one was 1, and SEVEN of the ten questions were about the same noun — a card
  // on a lending shelf, asked where it rests, where it sits, and which surface supports it. The criticism
  // surface carries the sibling instruction ("move to a new place in the text") and this one did not.
  // ⚠️ Named rather than counted, for the reason this file already records: "do not circle" is
  // unactionable and "you have pointed at the card three times" is.
  const circlingBlock = circling && circling.length
    ? `\n🔴 [You have already pointed at ${circling.map((w) => `"${w}"`).join(' and ')} in your recent questions. Point somewhere ELSE in her specification this time — a different thing of hers. Rotating what you ask ABOUT is not enough if you keep asking about the same object.]\n`
    : '';

  const openerBlock = banOpeners && banOpeners.length
    ? `\n[Do NOT begin with ${banOpeners.map((w) => `"${w}"`).join(' or ')} — your last question${banOpeners.length > 1 ? 's' : ''} began that way.]\n`
    : '';
  // THE HEAD, named to the model as well (6 Sep 2026). Naming the openers alone left the model free to
  // keep the template and change the first word, which is what a three-beat metronome is. Quoted, because
  // "do not repeat a construction" is unactionable and "you have begun a question 'Where does the'" is not.
  const headBlock = banHeads && banHeads.length
    ...
    : '';
  const frameBlock = avoidFrames && avoidFrames.length
    ? `\n[You have already asked, in these shapes: ${avoidFrames.slice(-4).map((f) => `"${f}"`).join('; ')}. Ask this one in a DIFFERENT construction.]\n`
    : '';

  // 🔴 TELL THE COMPOSING LAYER WHAT THE GUARD WILL REFUSE. This file's own 17 August lesson: a rule the
  // model has not been given is a repair loop rather than a guard. Every refusal in SPEC_FORBIDDEN is
  // stated here in the model's own terms, and the guard still enforces them.
  return `You ask questions about a specification. You never answer, explain, advise, evaluate or complete one.

A specification is a description, in ordinary language, of a thing that does not exist yet — complete enough that somebody or something else could build it. The student wrote this one. It is unfinished on purpose and she is going to change it.

Your whole job is to ask ONE question that makes her look at a part of her own specification.
${specBlock}${assignmentBlock}${jointBlock}${spentBlock}${circlingBlock}
HOW TO ASK
- ONE question. A single clause, one question mark. Never join a second on with "and".
- Ask what hers is, does, holds, decides, shows — or what it declines to know. Not what it should do.
- Plain words. Ask it the way a person would say it out loud. Not "by what mechanism", "by which criteria", "in what manner", "by what evidence" — those are not questions a first-year can answer, they are a form to fill in.
- ${JARGON_PROMPT_LINE} Words from her specification are hers and are fine.

POINTING WITH HER WORDS
- Use her nouns. If she wrote "card", ask about the card.
- Quote only when the quoted words are a WHOLE phrase that still reads correctly inside your sentence. "reaches the wall" torn out of her sentence and dropped into yours produces broken grammar. If it does not read aloud cleanly, use her word without the quote marks.
- Do not put quote marks in every question. Most questions need none.

🔴 THE ONE THING THAT MATTERS MOST — DO NOT PRESUPPOSE A MECHANISM SHE HAS NOT DESCRIBED.
This is subtler than naming an absence and it hands her more. "How does the box determine the duration of the delay?" invents a delay: she never wrote one, and the question tells her that her counter needs one and that the remaining problem is tuning it. "What does the screen show while it waits for that variable?" invents a variable.
Whatever your question takes for granted, she must already have written. If you find yourself about to ask HOW something works, check that she said it exists. If she did not, ask about the place instead: "what happens when two people walk past together?" takes nothing for granted and leaves the mechanism to her.

WHAT WILL BE REFUSED, so do not write it
- Naming an absence: "you have not said", "your spec doesn't mention", "this is missing", "there is no mention of".
- Instructing: "you should", "you need to", "consider adding", "don't forget", "have you considered".
- Grading it: calling the specification thorough, vague, weak, complete or incomplete.
- Any yes/no question, and any question offering a choice between two options.
${openerBlock}${headBlock}${frameBlock}${tailBlock}
Write only the question.`;
}

// 🔴 THIS SURFACE'S GUARD. Shares the FORM checks with the other two — one question, opener ban, frame
// repeat, no binary menu, no closed question, no invented words — and adds the refusals above.
// It deliberately does NOT run CRITICISM_FORBIDDEN or the self-defence patterns: on this surface asking
// a student about her own document is the point, not the drift.
export function validateSpecOutput(text, { maxWords = 0, avoid = [], banOpeners = [], banHeads = [],
  ownWords = null, noCompound = true, noJargon = true } = {}) {
  const framing = String(text).replace(/["“”][^"“”]*["“”]/g, ' ');   // her quoted words are hers, not the model's
  const reasons = [];
  if (!String(text).includes('?')) reasons.push('no question present');
  for (const re of FORBIDDEN) if (re.test(framing)) reasons.push(`forbidden pattern: ${re}`);
  for (const re of SPEC_FORBIDDEN) if (re.test(framing)) reasons.push(`completes the spec instead of asking about it: ${re}`);
  reasons.push(...sharedFormChecks(text, framing,
    { maxWords, avoid, banOpeners, banHeads, noBinary: true, noClosed: true, ownWords, noCompound, noJargon }));
  reasons.push(...possessiveInvention(text, ownWords));
  return { ok: reasons.length === 0, reasons };
}

// 🔴 "YOUR <thing she never wrote>" — the reviewer's tell, and the one shape `inventionReasons` cannot
// see. That function fires on a DEICTIC ("that gravity", "this timeout") and a possessive is not one, so
// «what does your gravity do to the ball?» passed every check while naming the exact thing she left out.
// On enquiry that is a wasted turn; here it is the whole failure mode, because the noun IS the answer.
//
// 🔴 IT IS ADDED HERE RATHER THAN TO THE SHARED FUNCTION ON PURPOSE. Widening `DEICTICS` would change
// what two shipped surfaces refuse, silently, to fix a third one's problem — the exact shape this file
// already records as a rule written on one surface arriving on another that never asked for it.
//
// ⚠️ WHAT IT CANNOT SEE, STATED RATHER THAN IMPLIED: a bare invented noun with no determiner in front of
// it — «what happens at the corner?» when she never wrote "corner". Catching that needs a general noun
// list, and a list that cannot be complete would refuse correct questions to catch some wrong ones. The
// system prompt names this refusal in the model's own terms, which is what reduces it; the guard's claim
// is only the breaches it can actually see.
function possessiveInvention(text, ownWords) {
  if (!(ownWords instanceof Set) || !ownWords.size) return [];
  const bare = stripQuoted(String(text)).toLowerCase();
  const toks = bare.match(/[a-z]+/g) || [];
  const known = [...ownWords];
  const theirs = (w) => ownWords.has(w)
    || (w.endsWith('s') && ownWords.has(w.slice(0, -1)))
    || ownWords.has(`${w}s`)
    || known.some((k) => k.length >= 6 && w.length >= 6 && k.slice(0, 6) === w.slice(0, 6));
  const POSS = new Set(['your', 'its', 'their']);
  // 🔴 THE SURFACE'S OWN VOCABULARY IS NOT AN INVENTION, and leaving it out refused the first correct
  // question written for this guard: «what does your SPECIFICATION say the speed becomes?» — flagged
  // because she had never used the word "specification", which of course she had not; it is the word for
  // the thing she wrote, not a thing inside it. These are words ABOUT the artefact and about answering,
  // and naming one supplies nothing she was supposed to supply.
  const META = new Set(['specification', 'specifications', 'spec', 'specs', 'document', 'documents',
    'description', 'descriptions', 'wording', 'sentence', 'sentences', 'version', 'versions', 'answer',
    'answers', 'account', 'writing', 'draft', 'drafts', 'note', 'notes', 'line', 'lines', 'words']);
  for (let i = 0; i < toks.length - 1; i++) {
    if (!POSS.has(toks[i])) continue;
    const w = toks[i + 1];
    // The same phrase-ending tests as inventionReasons, and in the same order, for the reason recorded
    // there: a short or benign or participial head noun ends the phrase rather than sending the scan
    // hunting further along for something to object to.
    if (FUNCTION_WORD.has(w) || DEICTIC_OK.has(w) || META.has(w)) continue;
    if (w.length < 5 || /(?:ing|ed)$/.test(w)) continue;
    if (theirs(w)) continue;
    return [`"${toks[i]} ${w}" names a thing she never wrote — on this surface the noun IS the answer, `
      + 'so point with a word from her own specification or ask what is there without naming it'];
  }
  return [];
}

// ---------------------------------------------------------------------------------------------------
// WOULD THIS BUILD? — the builder's report (3 September 2026)
//
// 🔴 THIS ACT DOES WHAT THE QUESTIONING SURFACE REFUSES TO DO, AND THE COLLISION IS NAMED RATHER THAN
// SMOOTHED OVER. Every other line on this surface exists so that an absence is found by the student:
// `SPEC_FORBIDDEN` refuses "you have not said", the aims move to a place rather than to a gap, and the
// joints reading is never rendered as a count because a figure about unfinished work is a mark. This
// returns a list of gaps. It was asked for directly — the specification has to be good enough that
// something can build from it, and nothing here could tell her whether it was.
//
// 🔴 WHAT KEEPS IT THE RIGHT SIDE OF THE POSITION IS THE POLARITY OF THE SENTENCE, and it is the whole
// design. The report is written as decisions THE BUILDER TAKES — "I would have to decide how often it
// checks" — never as things she failed to write. That is not a euphemism for the same content. A gap
// named as her omission is a verdict on her document; the same gap named as a decision somebody else is
// about to take is a fact about what happens next, which is the thing she cannot see and the reason the
// deck's whole argument holds: whatever she does not specify still gets decided, and this says by whom.
//
// ⚠️ IT IS A SEPARATE ACT, ON REQUEST, AND THE QUESTIONING IS UNTOUCHED. No turn of the conversation
// produces this, nothing computes it in the background, and it never arrives unasked. A student who
// never presses it has the surface exactly as it was.
//
// ⚠️ NO COUNT, NO SCORE, NO READY FLAG. It returns the decisions and nothing else — no total, no
// percentage, no "your specification is complete". A number here would be the mark this surface has
// refused to make since it was built.
export function buildBuilderPrompt({ spec = '', assignment = '' } = {}) {
  const assignmentBlock = assignment && assignment.trim()
    ? `\n== WHAT WAS ASKED FOR (context only, never a standard to judge against) ==\n"""\n${assignment.trim()}\n"""\n`
    : '';
  return `You are a builder. You have been handed the specification below and told to build the thing it describes. You cannot ask the author anything — she is not available, and you have to start today.
...
"""
${assignmentBlock}
List the decisions YOU would have to take yourself, because the specification does not settle them.

HOW TO WRITE EACH ONE
- As a decision you are taking. "I would have to decide how often it checks." "I would have to choose what happens the first time it runs, before there is anything to read."
- NEVER as something she failed to write. Not "you have not said", not "the spec is missing", not "this is unclear", not "you should specify". You are not reviewing her document; you are reporting what you are about to decide on her behalf.
- One decision per line, each beginning with "- ".
- Plain words. Say what you would decide, not what category it belongs to.
- Say what you would decide it TO, where you would have a default in mind: "I would have to decide how often it checks, and I would probably pick once a second."
- Between three and eight lines. If the specification genuinely settles almost everything, say so in one line and stop.

Do not praise it, do not grade it, do not summarise it, and do not ask any questions. Write only the list.`;
}

// The report's own guard. Same discipline as everywhere else here: the rule the model is given is also
// enforced in code, because a rule stated only in a prompt is a repair loop rather than a guard.
const BUILDER_FORBIDDEN = [
  /\b(you (have not|haven'?t|did not|didn'?t|need to|should|must)|your (spec|specification|document)\b)/i,
  /\b(is|are|it'?s) (missing|absent|unclear|vague|unspecified|undefined|incomplete)\b/i,
  /\b(consider (adding|specifying)|please (specify|define|clarify))\b/i,
];

// 🔴 IT DROPS THE OFFENDING LINE AND KEEPS THE REST, rather than refusing the whole report. One line
// slipping into review language does not make the other six wrong, and a report that vanishes because
// of its fourth line teaches nobody anything. What is dropped is counted for the caller, never shown.
export function validateBuildReport(text) {
  const lines = String(text || '').split('\n')
    .map((l) => l.replace(/^\s*[-*•]\s*/, '').trim())
    .filter((l) => l.length > 3);
  const kept = [], dropped = [];
  for (const l of lines) (BUILDER_FORBIDDEN.some((re) => re.test(l)) ? dropped : kept).push(l);
  return { items: kept.slice(0, 8), dropped: dropped.length };
}
