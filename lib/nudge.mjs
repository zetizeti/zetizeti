...
export const FLOW_SHAPES = [
  ...
  'Ask this one the way a friend would — plain and curious, no framing clause, no demand for precision. If something in what they said struck you, you may say so in one short clause first, then ask.',
];
...
export function decideNudge(sig = {}, state = {}, { warmth = false } = {}) {
  ...
  if (warmth && (sig.movement ?? 0) >= 0.15 && (sig.advancement ?? 0) >= 0.2 && turnsSinceNudge >= 1) {
    return {
      posture: 'Open with ONE short line naming what is WORKING in their line of thought — vary how you say it, and use THEIR words for the thing ("that distinction is doing real work", "this line is opening something", "now the real tension is in view", "that is a sharper way to put it"). It is about the INQUIRY they are developing, never about them as a person: never "you\'re doing well", never a grade. Then your single question.',
