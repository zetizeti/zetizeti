...
export function decideNudge(sig = {}, state = {}, { warmth = false } = {}) {
  ...
  if (warmth && (insightRoute || developmentRoute) && turnsSinceNudge >= 1) {
    return {
      // 9 September 2026: the two appraisal examples this line carried ("… is doing real work", "this line is
      // opening something") were being said back verbatim in real dialogues; see posture 6 below.
      posture: 'Open with ONE short clause that says back, in THEIR exact words, the specific thing they have just added or sharpened ("so the storefront is doing X" — their X, their verb). An acknowledgement, never a verdict: nothing about it working, opening, being clearer or interesting. It is about the INQUIRY, never about them as a person. Then your single question.',
  ...
  if ((sig.movement ?? 0) >= 0.3 && (sig.advancement ?? 0) >= 0.4) {
    return {
      // 🔴 REWRITTEN 9 September 2026 — the dial no longer VALUES anything. It read: open with a warm line
      // naming what is WORKING — "this line is opening something", "that distinction is doing real work",
      // "now the real tension is in view". Twenty real dialogues showed eight preambles that were those three
      // sentences said back verbatim, and under the artifact framing (CLAUDE.md, 7 September) a verdict on
      // what the learner's material is doing is a judgement on the material, which is theirs to make. What
      // survives is the acknowledgement: their own words said back, and then the question. The 24 July
      // "light encouragement" setting is retired by this; the ledger records who asked for it and why.
      posture: 'The learner has just moved their own thinking forward. You may open with ONE short clause that says back, in THEIR exact words, the thing they just named or distinguished — an acknowledgement and nothing more: no verdict on it, nothing about it working, opening, becoming clearer or being interesting — then your single question.',
      surface: null, fired: 'acknowledge',
    };
  }
