...
export function retrieve(db, text, { limit = 3, extraTerms = [], discipline = null, excludeIds = [], focus = null } = {}) {
  ...
  params.push(limit + excludeIds.length);   // over-fetch so exclusions can be backfilled to `limit`
  const rows = db.prepare(`
    ...
    LIMIT ?
  `).all(...params);
  if (!excludeIds.length) return rows.slice(0, limit);
  const ex = new Set(excludeIds);
  return rows.filter((r) => !ex.has(r.id)).slice(0, limit);
}
