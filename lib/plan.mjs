...
//
// 🔴 MEASURED 15 AUGUST 2026, AND IT CAME BACK NULL ON THE QUESTIONS. `scripts/plan-vs-clock.mjs`, four documents of differing affordance, four conversations of eight rounds per arm, 128 questions each, play-acted student permitted to disengage. Student disengagement: plan 15.6%, clock 16.4% — eight tenths of a point, about one question. Frame entropy: plan 1.83, clock 1.93 — the plan slightly LOWER, meaning marginally fewer kinds of question. Frame distributions near-identical. **The plan changes the route and does not change the questions in kind. That is the v0.11.0 result again, on this surface.** Do not claim, in a doc, a grant or a commit message, that the reading plan asks better questions. It has been tested and it does not.
//
// ⚠️ THE ARGUMENT THAT PREDICTED OTHERWISE IS KEPT SO IT IS NOT RE-DERIVED AS NEW. It ran: the v0.11.0 null need not transfer because the object differs — on the enquiry surface the subject is the learner's own edge and an imposed aim fights the material, whereas here it is a fixed external text with enumerable spots, so a plan is a reading order rather than a direction imposed on somebody's thinking. It is a good argument. It is also wrong, or at least too small to see at this power.
//
// 🔴 WHAT THE NULL DOES NOT TOUCH, AND WHY THIS FILE STAYS. Three claims were bundled together and only ONE was tested. (a) QUESTION QUALITY — tested, null. (b) COST — `windowOf` needs a live region to window to; with no plan there is no region, and the whole document re-enters the system prompt on every turn, which is what made the 25,000-character ceiling affordable in the first place. (c) COMPLETION — the clock never ends, so a critique could never be finished, and `turn_depth` could never tell finishing from leaving. (b) and (c) are load-bearing and untested-because-unarguable. Removing this file to honour the null would reintroduce a cost blowout and lose completion, to fix nothing.
//
// ⚠️ ONE DIRECTIONAL HINT, INSIDE THE NOISE AND WORTH ACTING ON ANYWAY. The plan asked the verdict frame 32 times to the clock's 23. `blur` is ordered first whenever the locator finds anything, which on every real document it does, so the plan opens on the verdict frame and dwells there — the precise complaint that created the pointer rotation (Siddhi, 16 July: it "constantly framing 'is this a property or a verdict' to whatever answer I give"). The plan may be quietly reinstating the fault the rotation was built to remove. Unmeasured; recorded here rather than fixed on a hunch.
//
// ⚠️ LIMITS OF THE MEASUREMENT, stated so nobody reads it as stronger than it is. 128 questions an arm cannot see a small effect; what it can say is that no large one is there. Half of all questions matched no frame pattern (`other` ≈ 52% on both arms), so the entropy figure is partly measuring the classifier's regexes — the disengagement figure is classifier-free and is the stronger of the two. And a model does not close the tab: none of this shows whether a real student would have stayed.
...
/**
 * The project brief, reduced to the part that frames anything.
 *
 * A project brief is mostly scene-setting; what makes it usable as CONTEXT is the handful of sentences establishing who it is for, what it is mainly trying to do, and what it commits to. Those are the same three lexicons the affordance rules already use, so the digest is a reuse rather than a new judgement — and taking the framing sentences beats taking the opening N characters, which is whatever the author happened to put first.
 *
 * ⚠️ IT IS AN EXTRACT AND THE PROMPT SAYS SO. A model handed a digest labelled as the whole brief will answer as though it has read the project. The block that carries this states that it is a extract, for the same reason the skeleton exists above.
 */
export function briefDigest(segments = [], { maxChars = 3000 } = {}) {
  const scored = [];
  for (const seg of segments) {
    const t = lower(seg.text);
    let score = 0;
