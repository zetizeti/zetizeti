...
function readAssociationCore({ studentTurns = [], stoneTurns = [], minGap = 3,
                               selector = 'charge', skipCorrected = false } = {}) {
  const n = studentTurns.length;
  if (selector !== 'charge') {   // 'recurrence' (the preserved A1 comparator) and 'open' (shipping)
    return pickJoin({ studentTurns, stoneTurns, minGap, used: new Set(), selector });
  }
  const used = new Set();
  let result = null;
  for (let m = minGap + 1; m <= n; m++) {
    ...
    const hit = gated ? null : pickJoin({
      ...
      minGap, used, selector,
    });
    if (m === n) { result = hit; break; }
    if (hit) used.add(hit.earlyIndex);
  }
  return result;
}
...
export function readAssociation(opts = {}) {
  const { studentTurns = [], stoneTurns = [], minGap = 3, spacing = false } = opts;
  const n = studentTurns.length;
  if (!spacing) return readAssociationCore(opts);
  let lastFired = -99;
  for (let m = minGap + 1; m <= n; m++) {
